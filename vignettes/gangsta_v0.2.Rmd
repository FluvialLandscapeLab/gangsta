...
The `gangsta` simulation model code is written to a file that can be run in lpSolve (http://lpsolve.sourceforge.net/5.5/).  Files should be saved with a ".lp" file extension.  An R interface to lpSolve is contained within the `lpSolveAPI` package.
```{r eval = FALSE}
